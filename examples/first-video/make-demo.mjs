...
const motions=['文案落版→录音进入→两份输入配对','输入汇入声音→配音波形生成→音色克隆落定','展开音轨→按真实 cue 拆分→时间标记落位→字幕与声轨对齐','时间轴进入网页→文字长成卡片→连接传递→主体焦点推进','同一时钟依次推动文案、声音、画面的焦点','收拢网页→演示播放→分出视频→完成落版'];
...
fs.writeFileSync(path.join(chapter,'waveform.ts'),'export const voiceModelLabel = '+JSON.stringify(({minimax:'MINIMAX',mlx:'QWEN3-TTS',qwen:'QWEN3-TTS',nano:'MOSS-TTS-NANO'})[m.provider]||m.provider)+';\nexport const waveformIsIllustrative = false;\nexport const waveform = '+JSON.stringify(bins.map(x=>Math.max(.06,Number((x/(peak||1)).toFixed(4)))))+';\n');
