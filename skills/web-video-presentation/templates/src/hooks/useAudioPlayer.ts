...
interface Options {
  ...
  autoStarted: boolean;
  /**
   * Muted playback (`?mute=1`). Audio still decodes and still fires `ended`,
   * so auto-advance timing is byte-for-byte identical to unmuted — you just
   * can't hear it.
   *
   * This exists for automated verification: an agent driving the page with
   * `?auto=1&mute=1` can screenshot key frames without blasting narration
   * out of the user's speakers. **Never pass it when recording.**
   */
  muted?: boolean;
}
...
export function useAudioPlayer({
  ...
  }, [src, mode, trailMs, estimateFallbackMs, autoStarted, muted]);
}
