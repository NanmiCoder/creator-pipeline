...
多个动作拍可共用一个 SRT step；每拍用自身的起点与时长采样。手动预览时将本 Step 的动作限制在它的区间内，避免预览时钟继续累加、尚未点击就演出后续 Step。内部 stagger 的最后一个对象也须在该拍结束前落定。配音步数、字幕数与动作拍数分别记录，不为增加动画去伪造字幕段。

如需要“下一步播放一个动作，然后停住”的演示入口，可提供显式开启的静音 review 控制器：它推动 `presentation:seek`，结束时停在动作落点，并支持回退与重播。控制器放在 16:9 内容框外，仅在 review 模式挂载；普通自动播放仍只由 audio.currentTime 驱动。章节本身保持纯时间采样，不另起独立计时器。减少动态效果时直接到落点。普通点击、自动播放和逐步预览分别实测。
